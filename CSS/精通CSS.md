...
如上，页面上元素原来的位置仍然保留了像素空间，但元素上的所有点都被畸变场给变换到了新位置。如果给变换后的元素再应用一个外边距，那么因为元素所在的整个据表坐标都会被旋转，不会出现跟未旋转时一样的效果。旋转后的元素也不会妨碍页面其他部分的布局，就好像根本没有变换过一样。但变换会影响页面的溢出。如果变换后的元素超出设置了 overflow 属性的元素，导致出现了滚动条，则变换后的元素会影响可滚动区域。
