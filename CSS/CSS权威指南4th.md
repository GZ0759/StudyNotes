...
通常， CSS 属性值的变化（发生“样式变化事件”）瞬间完成。属性的新值替换旧值持续的时间是毫秒级的，受影响的内容在这短暂的时间内重新绘制，若有必要，先重排布局再重新绘制。多数情况下，值的变化是瞬间完成的，整个过程不超过16毫秒注。即便所用的时间较长，也是从一个值直接变为另一个值。例如，鼠标悬停时改变背景色，背景直接从一个颜色变为另一个颜色，中间没有过渡。
...
在CSS中，过渡使用四个属性定义：transition-property、transition-duration、transition-timing-function和transition-delay。此外，还有个简写属性transition,可一次声明全部四个属性。
...
默认情况下没有过渡效果，然而如果设置了过渡，而后又想在特定的情况下撤销过渡效果，可以使用`transition-property: none`覆盖整个过渡声明，金庸所有属性的过渡效果。

**过渡事件**

在DOM中，不管是哪个方向的过渡，不管过渡持续多久，延迟多长，也不管过渡的属性是单独声明还是覆盖在 all 中，过渡结束后都会触发transitionend事件，有时看似单个声明的属性，却会触发多个transitioned事件，因为简写属性中每个支持动画的属性有各自的transitionend事件。以下述样式为例：

### 17.2.2 设置过渡持续时间

transition-duration属性的值是以逼号分隔的时间长度列表，单位为秒（s)或毫秒(ms)。这些值指定从一个状态过渡到另一个状态历时多久。

### 17.2.3 调整过渡的内部时序

你是否希望过渡慢速开始，然后逐渐加快？或者快速开始，逐渐减速走向终点？又或者先平稳行进，然后步进甚至弹跳数次？transition-timing-function属性用于控制过渡的步调。
...
如果只在全局状态中声明过渡，鼠标悬停和移开状态使用相同的transition声明，因为选择符能匹配两个状态。如果不想完全复用整个过渡，覆盖部分过渡的属性，可以在态（而不只是悬停状态）中声明不同的过渡值。
