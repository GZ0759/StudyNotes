...
主动提交。VCS 和文本编辑器的撤销功能比起来，有一个很重要的区别是：程序代码的修改的生命周期非常长。 VCS 保存修改历史，使用的是主动提交改动的机制。在写完一段完整的代码之后，使用 commit 命令把改动和对改动的描述信息提交，这样就能记录到版本历史当中了。

中央仓库。中央仓库作为代码的存储中心，所有人的改动都会上传到这里，所有人也都能看到和下载别人上传的改动，解决同步的需求。
...
# 3 上手 1：快速上手 Git 管理代码

目录 `.git` 就是本地仓库（Local Repository），所有版本信息都会存在这里。而 `.git` 所在的这个根目录，称为 Git 的工作目录（Working Directory），它保存了你当前从仓库中签出（checkout）的内容。

stage 这个词在 Git 里，是「集中收集改动以待提交」的意思。所谓的 staging area，是 `.git` 目录下一个叫做 `index` 的文件。通过 `git add` 指令暂存的内容，都会被写进这个文件里。

# 4 上手 2：团队工作的基本工作模型

写完所有的提交后，不用考虑中央仓库是否有新的提交，直接提送上去就好。如果推送失败，就用通过拉取命令把本地仓库的提交和中央仓库的提交进行合并，然后再推送一次。
...
如果你想在某处创建分支 ，只需要输入一行 `git branch 名称`。不过新建的分支并不会自动切换，你的 HEAD 在这时依然是指向主分支的。你需要通过签出来主动切换到你的新分支去。然后 HEAD 就会指向新建的分支了。除此之外，你还可以用 `git checkout -b 名称` 来把上面两步操作合并执行。这行代码可以帮你用指定的名称创建分支后，再直接切换过去。
...
2. 特殊情况：HEAD 领先于目标 commit

如果合并时的目标提交和 HEAD 处的提交并不存在分叉，而是 HEAD 领先于目标提交。那么合并就没必要再创建一个新的提交来进行合并操作，因为并没有什么需要合并的。在这种情况下， Git 什么也不会做，合并是一个空操作。
...
这种 Feature Branching 工作流的核心内容可以总结为两点：  
...
1. tag：不可移动的 branch。tag 是一个和 branch 非常相似的概念，它和 branch 最大的区别是：tag 不能移动。所以在很多团队中，tag 被用来在关键版本处打标记用。
2. cherry-pick：把选中的 commits 一个个合并进来。cherry-pick 是一种特殊的合并操作，使用它可以点选一批 commits，按序合并。
3. git config： Git 的设置。git config 可以对 Git 做出基础设置，例如用户名、用户邮箱，以及界面的展示形式。内容虽然多，但都不难。
4. Git Flow：复杂又高效的工作流。Git Flow 的机制非常完善，很适合大型团队的代码管理。